  ...
  try {
    ...
    if (!process.env.POE_API_KEY) {
      return NextResponse.json(
        {
          message: "PyVax AI is not configured. Please add your Poe API key to use this feature.",
    ...
    if (!response.ok) {
      ...
      if (response.status === 402) {
        errorMessage =
          "PyVax AI requires a Poe subscription or credits. Please check your Poe account status at poe.com."
